...
export class HealthCheckService {
  ...
  public addDataListeners() {
    this.hupConnection.on('Update', (msg) => {
      console.log("Update issued by server for the following reason: " + msg);
      this.updateData();
    })
    this.hupConnection.on('ClientUpdate', (msg) => {
  ...
  public updateData() {
    ...
    this.http.get<Result>(environment.baseUrl + 'api/health')
      .subscribe(result => {
        this._result.next(result); console.log(result);
      });
  }

  public sendClientUpdate() {
    this.hupConnection.invoke('ClientUpdate', 'client test')
